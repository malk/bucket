...
#ansible-playbook -i linsharefull-example linshare.yml
echo "### Create resources directory ###"
[ -d resources ] || mkdir resources
pushd resources
echo "### Copy yum cache to /root/6 ###"
ssh linshare.example.com "cp -r /var/cache/yum/x86_64/6 /root/"
echo "### Create RPM repositories ###"
ssh linshare.example.com "find /root/6 -type d -name packages -exec createrepo -q {} \;"
echo "### Sync repos from remote host ###"
rsync -r --delete linshare.example.com:/root/6/* rpm-repos
...
echo "### Download linshare-release RPM ###"
if [ ! -f linshare-release.noarch.rpm ]; then
    wget -q http://packages.linshare.org/rpm/25/release/linshare-release.noarch.rpm
fi
